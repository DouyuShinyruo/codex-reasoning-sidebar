# Changelog

## [Unreleased]

### Added

- 悬浮窗图钉按钮：控制是否始终置顶（默认置顶，状态持久化）
- 历史追补：服务端保留最近 300 条环形缓冲，新连接的客户端立即获得当前会话历史（即时渲染，不打字机），并滚动到最新
- SSE 连接建立时推送当前会话标签（此前晚连接的客户端收不到「已监听」通知）

### Fixed

- 会话活跃检测纳入 mtime 变化：修复已加载的 idle 会话之间切换时悬浮窗不跟随的问题（聚焦 idle 会话只触碰文件不追加内容，此前的大小对比检测不到）
- 聚焦意图越过粘滞保护：idle 会话被聚焦后立即切换，且保持跟随约 30 秒（`FOCUS_LOCK_MS` 可调），期间其他会话的高频写入不会抢走

### Changed

- 悬浮窗宿主由 pywebview 迁移到 Electron：四边四角原生拖拽缩放、单实例防重复启动、尺寸位置持久化；移除 Python 运行时依赖
- Electron 运行时独立安装在 `%LOCALAPPDATA%\codex-reasoning-sidebar`，插件目录与市场缓存保持轻量

### Added

- 悬浮窗模式：`float-sidebar.pyw` / `start-codex-reasoning-sidebar-float.ps1`，无边框置顶可拖动窗口，自动拉起本地服务
- 悬浮窗右下角缩放手柄：拖动调整宽高，尺寸会被记住
- 工具调用索引条与页内查找（`Ctrl+F`）：点击/搜索可定位并高亮对应条目
- 查找支持 VSCode 风格三开关：区分大小写 / 全字匹配 / 正则表达式（含 `Alt+C` / `Alt+W` / `Alt+R` 快捷键、错误正则提示、当前项高亮区分）
- 缩放手柄改为原子窗口边界更新 + 按显示帧提交，修复拖动时尺寸跳变（此前存在逻辑/物理像素双重换算问题）
- 页内查找入口改为仅 `Ctrl+F` 快捷键，顶栏不再显示「查找」按钮
- 悬浮窗改用固定持久化 WebView2 数据目录，主题/尺寸设置真正跨重启保留
- 启动脚本自动探测可用的 Python 解释器；在服务类非交互会话中运行时，通过一次性计划任务转发到用户桌面会话启动（修复会话 0 无法启动 WebView2 的问题）
- 浅色主题：默认跟随系统，可手动切换 自动 / 浅色 / 深色，选择会记住
- 会话信息现在显示项目名 + 短会话 ID（从 `session_meta` 解析）

### Fixed

- 四种消息类型（思考 / 回复 / 工具 / 我）默认全部勾选
- 会话自动跟随改为基于文件实际写入（大小变化）而非仅 mtime，多窗口切换更可靠
- 切换会话时不再从头回放整段历史，只回放最近约 64KB，切换即时生效
- 修正会话 ID 提取错误的问题（此前正则会匹配到时间戳片段）
- 修复停止脚本使用保留变量 `$pid` 导致无法停止服务的问题
- `REPLAY_TAIL_KB` 环境变量：控制启动/切换会话时回放多少历史（默认 64KB，`0` 为全部）

### Changed

- 许可证由 MIT 调整为 Creative Commons Attribution-NonCommercial 4.0 International（非商业使用）`n- 许可证由 CC BY-NC 4.0 调整为 GPL-3.0（GNU General Public License v3）

## [0.1.0] - 2026-07-31

### Added

- 首个正式版本
- 本地 HTTP + SSE 服务，自动跟随最近活跃的 Codex 会话
- 打字机式流式渲染，支持点击正文立即显示全文
- 支持按类型过滤：思考、回复、工具调用、用户消息
- 支持暂停、继续与清空
- 内置 30 秒去重与 300 条记录上限
- 提供 PowerShell 安装、启动、停止脚本
