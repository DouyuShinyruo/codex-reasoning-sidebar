...
description: Show the current Codex window's live reasoning chain in the in-app browser side panel. Use when the user asks to open, start, stop, or check the reasoning sidebar.
---
...
1. Run `powershell -NoProfile -ExecutionPolicy Bypass -File "<plugin>/start-reasoning-sidebar.ps1"`.
...
Run `powershell -NoProfile -ExecutionPolicy Bypass -File "<plugin>/stop-reasoning-sidebar.ps1"`.
